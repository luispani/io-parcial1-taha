...
(function (global) {
  ...
  function arrow2d(ctx, x1, y1, x2, y2, color, width) {
    if (!ctx) return;
    var dx = x2 - x1, dy = y2 - y1;
    var len = Math.sqrt(dx * dx + dy * dy) || 1;
    var ux = dx / len, uy = dy / len;
    var head = Math.min(14, len * 0.35);
    ...
    ctx.beginPath();
    ctx.moveTo(x2, y2);
    ctx.lineTo(hx - uy * head * 0.45, hy + ux * head * 0.45);
    ctx.lineTo(hx + uy * head * 0.45, hy - ux * head * 0.45);
    ctx.closePath();
    ctx.fillStyle = color;
    ctx.fill();
  ...
  Engine.prototype.arrow = function (a, b, color, width) {
    var pa = this.P(a[0], a[1], a[2]);
    var pb = this.P(b[0], b[1], b[2]);
    arrow2d(this.ctx, pa.x, pa.y, pb.x, pb.y, color, width);
  };
  ...
  var LEGENDS = {
    ...
    'dual-balance': {
      what: 'Dualidad en Reddy Mikks: z* = w* = 21.',
      how: 'Se lee de izquierda a derecha. La flecha dice que en el óptimo el primal y el dual valen lo mismo. No gira.',
  ...
  function drawReddySurface(eng, t, walk) {
    ...
    if (walk) {
      ...
      for (i = 0; i < SIMPLEX_PATH.length - 1; i++) {
        eng.arrow(wr(SIMPLEX_PATH[i].x, SIMPLEX_PATH[i].y), wr(SIMPLEX_PATH[i + 1].x, SIMPLEX_PATH[i + 1].y), accent, 3);
      }
  ...
  function drawScenePipeline(eng) {
    var ctx = eng.ctx;
    if (!ctx) return;
    var w = eng._cssW || 640, h = eng._cssH || 360;
    var ink = cssVar('--ink', '#1F2A44');
  ...
  function drawSceneDual(eng) {
    ...
    var accent = cssVar('--accent', '#C77D1A');
    var ok = cssVar('--ok', '#2E7D5B');
    var boxW = Math.min(220, w * 0.32);
    var boxH = 110;
    var y = h / 2 - boxH / 2;
    var xL = w * 0.12;
    var xR = w * 0.88 - boxW;
    function card(x, title, lines, color) {
      ctx.fillStyle = paper;
      ctx.strokeStyle = color;
      ctx.lineWidth = 2;
      ctx.beginPath();
      ...
      ctx.textAlign = 'center';
      ctx.fillText(title, x + boxW / 2, y + 32);
      ctx.fillStyle = ink;
      ctx.font = '13px "Source Sans 3", sans-serif';
      var i;
      for (i = 0; i < lines.length; i++) ctx.fillText(lines[i], x + boxW / 2, y + 56 + i * 18);
    }
    card(xL, 'Primal (max)', ['z = 5x1 + 4x2', 'óptimo z* = 21'], ok);
    card(xR, 'Dual (min)', ['w = 24y1 + 6y2 + y3 + 2y4', 'óptimo w* = 21'], accent);
    arrow2d(ctx, xL + boxW + 8, y + boxH / 2, xR - 8, y + boxH / 2, accent, 3);
    ctx.fillStyle = ink;
    ctx.font = '700 14px "Source Sans 3", sans-serif';
    ctx.textAlign = 'center';
    ctx.fillText('z* = w* = 21', w / 2, y - 16);
    ctx.font = '12px "Source Sans 3", sans-serif';
    ctx.fillText('dualidad fuerte', w / 2, y);
    eng.caption('Reddy Mikks: el primal y el dual valen lo mismo en el óptimo.');
  }
